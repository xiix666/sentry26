...
修改后的定位链路显著降低了计算和内存占用，为同一计算设备上的相机感知和Nav2保留资源。新增地图点需要经过跨帧重复观测确认后才会写入地图，以减少瞬时噪声对地图的污染。当前优化主要围绕地图维护、增量插入、点云预处理和非必要输出展开，具体实现仍在继续整理和验证，因此本文档暂不展开到代码级细节。
...
该过滤用于阻止串口、驱动或传感器瞬时异常将非法数值传播到滤波器。相比在状态已经发散后恢复，在输入边界直接拒绝异常数据更简单，也更符合实车稳定性优先的目标。
