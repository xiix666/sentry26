...
public:
    SpecialArea(const rclcpp::NodeOptions &node_options) : Node("special_area", node_options)
    {
        ...
        // this->declare_parameter("left_top_x", 0.42);
        // this->declare_parameter("left_top_y", -7.69);
        // this->declare_parameter("right_bottom_x", 5.52);
        // this->declare_parameter("right_bottom_y", -6.2);
...
private:
    ...
    const double special_reset_interval_sec_ = 10 * 60.0;  // 10 分钟
    // const double special_reset_interval_sec_ = 10; 
    ...
    void checkRobotPosition()
    {
        ...
        if (sec - special_last_reset_sec_ >= special_reset_interval_sec_)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Special area: 10min timeout -> RESET status.");
        }
        ...
        try {
            ...
            // ====================== 第三区域滞回判断 ======================
            bool now_third_inside_small = (robot_x > third_min_x_ && robot_x < third_max_x_ &&
                                          robot_y > third_min_y_ && robot_y < third_max_y_);
            bool now_third_inside_large = (robot_x > third_hysteresis_min_x_ && robot_x < third_hysteresis_max_x_ &&
                                          robot_y > third_hysteresis_min_y_ && robot_y < third_hysteresis_max_y_);
            ...
            // }else if (special_inside_active_ && !special_inside_large) {
            //     special_inside_active_ = false;
            // }
            const double now_sec = this->now().seconds();

            // 第一次进入 special 小区域：开始发 2，并开始计时
            if (special_inside_small && !special_area_has_triggered_) {
                special_inside_active_ = true;
                special_area_has_triggered_ = true;
            ...
            // 已经处于 special 激活状态时，判断是否退出或者超时
            if (special_inside_active_) {
                const bool special_exited = !special_inside_large;

                const bool special_timeout =
                    (now_sec - special_trigger_start_sec_) >= special_active_duration_sec_;
                ...
                if (special_exited || special_timeout) {
                    ...
                    // RCLCPP_INFO(
                    //     this->get_logger(),
                    //     "Special area stopped. reason=%s, elapsed=%.2f sec",
                    //     special_timeout ? "timeout" : "exit",
                    //     now_sec - special_trigger_start_sec_);
                }
            }
            // ====================== 最终状态判断（关键修改） ======================
            int area_status = 0;
