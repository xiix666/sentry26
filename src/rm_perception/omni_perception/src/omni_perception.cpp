...
public:
  OmniPerception() : Node("OmniPerception") {
    ...
    enable_motion_compensation_ =
        this->get_parameter("enable_motion_compensation").as_bool();
    ...
    angle_pub_ = this->create_publisher<geometry_msgs::msg::Point32>(
        "/angle_pack", rclcpp::SensorDataQoS());

    send_to_lidar_pub_ =
        this->create_publisher<rm_interfaces::msg::SendToLidarMsg>(
            "/send_to_lidar_pack", 10);
  ...
  void cameraCallback(const rm_interfaces::msg::CameraMsg::SharedPtr &msg) {
    ...
    buffer_index_ = (buffer_index_ + 1) % 3;
    ...
    auto updateTarget = [&](int &id, std::array<float, 2> &enemy_pose_xy,
                            ...
                            const std::array<float, 5> &pose_x,
                            const std::array<float, 5> &pose_y) {
      for (int i = 0; i < 5; ++i) {
        if (id_list[priority_idx_sort[i]] == 0) {
          if (pose_x[priority_idx_sort[i]] > 0.01 &&
              pose_y[priority_idx_sort[i]] > 0.01) {
            id = 0;
            enemy_pose_xy[0] = 0.0;
            enemy_pose_xy[1] = 0.0;
            return;
    ...
    for (int i = 0; i < 4; ++i) {
      enemy_pose_x[i] = (i == 0)   ? msg->enemy_pose_1_x
                        : (i == 1) ? msg->enemy_pose_2_x
                        : (i == 2) ? msg->enemy_pose_3_x
                                   : msg->enemy_pose_4_x;
      enemy_pose_y[i] = (i == 0)   ? msg->enemy_pose_1_y
                        : (i == 1) ? msg->enemy_pose_2_y
                        : (i == 2) ? msg->enemy_pose_3_y
                                   : msg->enemy_pose_4_y;
    }

    updateTarget(ids_[0], enemy_pose_xy[0], msg->id_1, msg->enemy_pose_1_x,
                 msg->enemy_pose_1_y);
    updateTarget(ids_[1], enemy_pose_xy[1], msg->id_2, msg->enemy_pose_2_x,
                 msg->enemy_pose_2_y);
    updateTarget(ids_[2], enemy_pose_xy[2], msg->id_3, msg->enemy_pose_3_x,
                 msg->enemy_pose_3_y);
    updateTarget(ids_[3], enemy_pose_xy[3], msg->id_4, msg->enemy_pose_4_x,
                 msg->enemy_pose_4_y);
  }

  float getFrameTargetZ(const rm_interfaces::msg::CameraMsg &frame,
                        int group_idx) {
    static const int priority_idx_sort[5] = {0, 1, 4, 2, 3};
    const auto &id_list = (group_idx == 0)   ? frame.id_1
                          : (group_idx == 1) ? frame.id_2
                          : (group_idx == 2) ? frame.id_3
                                             : frame.id_4;
    const auto &pose_z = (group_idx == 0)   ? frame.enemy_pose_1_z
                         : (group_idx == 1) ? frame.enemy_pose_2_z
                         : (group_idx == 2) ? frame.enemy_pose_3_z
                                            : frame.enemy_pose_4_z;
  ...
  bool getBigGimbalPose() {
    try {
      auto transform = tf_buffer_->lookupTransform(
          aim_odom_frame_, gimbal_frame_, tf2::TimePointZero,
      ...
      gimbal_transform_.setOrigin(
          tf2::Vector3(gimbal_pose_x, gimbal_pose_y, 0.0));
  ...
  void convertSpeedToGimbalFrame(double odom_vx, double odom_vy,
                                 double &gimbal_vx, double &gimbal_vy) {
  ...
  void publishGimbalFrameCameraMsg() {
    ...
    auto convertArrayToMap =
        [&](const std::array<int, 5> &ids, const std::array<float, 5> &in_x,
            const std::array<float, 5> &in_y, std::array<float, 5> &out_x,
            std::array<float, 5> &out_y) {
          for (int i = 0; i < 5; ++i) {
            if (ids[i] == 0 ||
                (std::fabs(in_x[i]) < 1e-6 && std::fabs(in_y[i]) < 1e-6)) {
              out_x[i] = 0.0f;
              out_y[i] = 0.0f;
              continue;
            }
            ...
            double map_x = 0.0;
            double map_y = 0.0;
            convertLocalTargetToMap(map_to_gimbal_tf, local_x, local_y, map_x,
                                    map_y);
  ...
  void convertToGimbalFrame(double odom_x, double odom_y, double &gimbal_x,
                            double &gimbal_y) {
  ...
  std::array<rm_interfaces::msg::CameraMsg, 3> getLast3Frames() const {
    ...
    result[2] = camera_data_buffer_[buffer_index_];
    return result;
  }

  bool getMapToGimbalTransform(tf2::Transform &map_to_gimbal_tf) {
    try {
      auto tf_msg = tf_buffer_->lookupTransform(map_frame_, "base_link",
                                                tf2::TimePointZero,
                                                std::chrono::milliseconds(200));
      tf2::Quaternion q(
  ...
  void convertLocalTargetToMap(const tf2::Transform &map_to_local_tf,
                               double local_x, double local_y, double &map_x,
                               double &map_y) {
  ...
  std::tuple<int, std::pair<float, float>, int>
  getFrameHighestPriorityTarget(const rm_interfaces::msg::CameraMsg &frame) {
  ...
  void clearLast3Frames() {
    ...
    RCLCPP_DEBUG(this->get_logger(),
                 "Cleared last 3 frames cache successfully");
  }
  ...
  void calculateAngle() {
    ...
    int global_highest_priority_id = -1;
    int global_highest_level = 999;
    float target_x = 0.0f, target_y = 0.0f, target_z = 0.0f;
    int target_camera_group = -1;
    int camera0_frame_count = 0;

    for (int frame_idx = 2; frame_idx >= 0; --frame_idx) {
      const auto &frame = last_3_frames[frame_idx];
      auto [frame_priority_id, frame_pose, frame_group] =
          getFrameHighestPriorityTarget(frame);
      ...
      if (frame_group == 0 && frame_priority_id != -1 &&
          frame_priority_id != 0) {
        camera0_frame_count++;
      }
      if (frame_priority_id == -1 || frame_priority_id == 0 ||
          (frame_target_x == 0.0f && frame_target_y == 0.0f)) {
        continue;
      }
    ...
    if (global_highest_priority_id == -1) {
      RCLCPP_WARN(this->get_logger(),
                  "No valid target in latest frame, publish 0 angle");
      geometry_msgs::msg::Point32 msg;
      msg.x = 0.0f;
      msg.y = 0.0f;
      msg.z = 0.0f;
    ...
    RCLCPP_INFO(
        this->get_logger(),
        "Valid frames: %d, Latest frame highest priority ID: %d, pose: (%.2f, "
        "%.2f), angle: %.2f degrees",
        valid_frame_count, global_highest_priority_id, target_x, target_y,
        angle_deg);
    geometry_msgs::msg::Point32 msg;
    msg.x = static_cast<float>(yaw_deg);
    msg.y = static_cast<float>(pitch_deg);
    msg.z = 0.0f;
    angle_pub_->publish(msg);
  }

  void convertGimbalFrameToMap(double gimbal_x, double gimbal_y, double &map_x,
                               double &map_y) {
  ...
  void compensateTargetInGimbalFrame(double &target_gimbal_x,
                                     double &target_gimbal_y) {
  ...
  void PoseToLidar() {
      ...
      for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 5; ++j) {
          if (enemy_pose_x[i][j] != 0.0f && enemy_pose_y[i][j] != 0.0f) {
            ...
            float rotated_x =
                enemy_pose_x[i][j] * cos_yaw - enemy_pose_y[i][j] * sin_yaw;
            float rotated_y =
                enemy_pose_x[i][j] * sin_yaw + enemy_pose_y[i][j] * cos_yaw;

            enemy_poses_x_to_lidar[j] =
                rotated_x + self_pose_x + transform_red_x_;
            enemy_poses_y_to_lidar[j] =
                rotated_y + self_pose_y + transform_red_y_;
          }
        }
      }
    } else {
      for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 5; ++j) {
          if (enemy_pose_x[i][j] != 0.0f && enemy_pose_y[i][j] != 0.0f) {
            ...
            enemy_poses_x_to_lidar[j] =
                transform_blue_x_ - rotated_x - self_pose_x;
            enemy_poses_y_to_lidar[j] =
                transform_blue_y_ - rotated_y - self_pose_y;
          }
        }
      }
    }
    ...
    std::fill(enemy_poses_x_to_lidar.begin(), enemy_poses_x_to_lidar.end(),
              0.0f);
    std::fill(enemy_poses_y_to_lidar.begin(), enemy_poses_y_to_lidar.end(),
              0.0f);
  }
...
private:
  ...
  rclcpp::Publisher<rm_interfaces::msg::SendToLidarMsg>::SharedPtr
      send_to_lidar_pub_;
  rclcpp::Publisher<rm_interfaces::msg::CameraMsg>::SharedPtr
      gimbal_camera_pub_;
  ...
  std::array<rm_interfaces::msg::CameraMsg, 3> camera_data_buffer_;
  int buffer_index_ = -1;
