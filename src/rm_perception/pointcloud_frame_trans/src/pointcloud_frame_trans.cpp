...
public:
  PointCloudFrameTrans() : Node("pointcloud_frame_trans") {
    ...
    this->declare_parameter("limit_x", 10.0);
    this->declare_parameter("limit_y", 10.0);
    this->declare_parameter("limit_z", 3.0);
    ...
    pub_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>(output_topic_, 5);
    if (tran_odom_) {
      pub_odom_ =
          this->create_publisher<nav_msgs::msg::Odometry>(odom_out_, 15);
      sub_odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
          odom_in_, odom_qos,
          std::bind(&PointCloudFrameTrans::odomCallback, this,
                    std::placeholders::_1));
    }
    pub_base_odom_ =
        this->create_publisher<nav_msgs::msg::Odometry>(odom_base_out_, 5);
    std::cout << "tran节点启动" << std::endl;
    sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        input_topic_, 15,
        std::bind(&PointCloudFrameTrans::pointCloudCallback, this,
                  std::placeholders::_1));
  }
...
private:
  void pointCloudCallback(
    ...
    try {
      ...
      RCLCPP_WARN(this->get_logger(), "Failed to transform point cloud: %s",
                  ex.what());
    }
  }

  void publishTfTransform(const tf2::Transform &transform,
  ...
  void odomCallback(const nav_msgs::msg::Odometry::ConstSharedPtr odom_msg) {
    if (!tf_initialized_) {
      try {
        ...
        Eigen::Isometry3d eigen_transform_d =
            tf2::transformToEigen(transform_stamped);
        T_target_source = eigen_transform_d.cast<float>().matrix();
        tf_initialized_ = true;
      } catch (tf2::TransformException &ex) {
        RCLCPP_WARN(this->get_logger(), "TF lookup failed: %s Retrying...",
                    ex.what());
        return;
      }
    }
    ...
    tf2::Transform tf_baseodom_to_lidar =
    ...
    tf2::Transform tf_baseodom_to_base =
        tf_baseodom_to_lidar * tf_lidar_to_base_;
    ...
    out_base.pose.pose.orientation =
        tf2::toMsg(tf_baseodom_to_base.getRotation());
    {
      std::lock_guard<std::mutex> lock(odom_mutex_);
    ...
    tf2::Vector3 v_lidar(odom_msg->twist.twist.linear.x,
                         odom_msg->twist.twist.linear.y,
                         odom_msg->twist.twist.linear.z);
    tf2::Vector3 w_lidar(odom_msg->twist.twist.angular.x,
