...
IcpNode::IcpNode(const rclcpp::NodeOptions &options)
  ...
  try {
    ...
    initial_pose_.orientation.w = q.getW();
    initial_pose_.orientation.x = q.getX();
    initial_pose_.orientation.y = q.getY();
    initial_pose_.orientation.z = q.getZ();
...
void IcpNode::initialPoseCallback(
  ...
  if (!success_) {
    ...
    std::lock_guard lock(mutex_);
    map_to_odom_.transform.translation.x = initial_guess(0, 3);
    map_to_odom_.transform.translation.y = initial_guess(1, 3);
    map_to_odom_.transform.translation.z = initial_guess(2, 3);

    Eigen::Matrix3d rotation = initial_guess.block<3, 3>(0, 0);
