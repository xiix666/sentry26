...
const double MAX_ANGULAR_VEL = 30.0;    // 最大角速度 rad/s（比如5rad/s≈286°/s）
const double MAX_LINEAR_ACC = 2.0 * 9.81; // 最大线加速度 m/s²（2倍重力加速度）
const double QUAT_NORM_TOL = 0.1;      // 四元数模长容忍度（1±0.1）
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  ...
  if ((lidar_type == VELO16 || lidar_type == OUST64 || lidar_type == HESAIxt32) && cut_frame_init) {
    ...
    p_pre->process_cut_frame_pcl2(msg, ptr, timestamp_lidar, cut_frame_num, scan_count); //得到每个点时间戳
...
// // 校验四元数（可选，若使用姿态数据）
// double quat_norm = sqrt(pow(msg_in->orientation.x, 2) + pow(msg_in->orientation.y, 2) +
//                      pow(msg_in->orientation.z, 2) + pow(msg_in->orientation.w, 2));
// if (fabs(quat_norm - 1.0) > QUAT_NORM_TOL) {
// RCLCPP_WARN(rclcpp::get_logger("li_initialization"), "IMU quaternion norm abnormal: %.4f (should be ~1.0)", quat_norm);
// return; // 过滤异常四元
