...
std::string lid_topic, imu_topic,save_path;
...
bool enable_prior_pcd,use_batch;
...
void readParameters(std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  try {
    nh->declare_parameter<bool>("use_batch", true);
    nh->get_parameter("use_batch", use_batch);
    ...
    nh->declare_parameter<string>("pcd_save.save_path","");
    nh->get_parameter("pcd_save.save_path", save_path);
    ...
    nh->declare_parameter<string>("publish.init_frame", "odom_init");
    nh->get_parameter("publish.init_frame", init_frame);

    nh->declare_parameter<string>("publish.lidar_frame", "lidar_link");
    nh->get_parameter("publish.lidar_frame", lidar_frame);

    nh->declare_parameter<string>("publish.lidar_odom_frame", "lidar_odom");
    nh->get_parameter("publish.lidar_odom_frame", lidar_odom_frame);
