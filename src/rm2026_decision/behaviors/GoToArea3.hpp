...
/**
 * @brief 前往区域3的异步行为节点
 * 导航到区域3并持续监控导航状态，距离目标点<=0.4m视为到达
 */
class GoToArea3 : public BT::StatefulActionNode
{
public:
    GoToArea3(const std::string& name, const BT::NodeConfig& config)
    ...
    {
        return {
            BT::InputPort<geometry_msgs::msg::PoseStamped>("target_pose", "Area3 position"),
    ...
    BT::NodeStatus onStart() override
    {
        ...
        if (!node_input) {
            RCLCPP_ERROR(rclcpp::get_logger("GoToArea3"), "Failed to get ROS2 node instance");
        ...
        if (!target_pose) {
            RCLCPP_ERROR(node_->get_logger(), "Failed to get Area3 target position: target_pose");
            return BT::NodeStatus::FAILURE;
        }
        ...
        RCLCPP_INFO(node_->get_logger(), "Start navigating to area3 (x=%.2f, y=%.2f), tolerance=%.2fm",
    ...
    BT::NodeStatus onRunning() override
    {
        ...
        if (std::chrono::steady_clock::now() > timeout_point_) {
            RCLCPP_WARN(node_->get_logger(), "Navigation to Area3 timed out");
            return BT::NodeStatus::FAILURE;
        }

        // 仅按当前点到目标点距离判断到达（不依赖 navigation/status）
        if (checkDistanceToTarget()) {
            RCLCPP_INFO(node_->get_logger(), "Reached Area3");
    ...
    {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Navigation to Area3 interrupted");
        } else {
            RCLCPP_INFO(rclcpp::get_logger("GoToArea3"), "Navigation to Area3 interrupted (node not initialized)");
        }
        navigation_started_ = false;
    }
