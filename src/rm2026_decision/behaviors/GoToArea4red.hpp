...
/**
 * @brief 前往区域4红色点的异步行为节点
 * 导航到区域4红色点并持续监控导航状态，距离目标点<=0.2m视为到达
 */
class GoToArea4red : public BT::StatefulActionNode
{
public:
    GoToArea4red(const std::string& name, const BT::NodeConfig& config)
    ...
    {
        return {
            BT::InputPort<geometry_msgs::msg::PoseStamped>("target_pose", "Area4red position"),     
    ...
    BT::NodeStatus onStart() override
    {
        ...
        if (!node_input) {
            RCLCPP_ERROR(rclcpp::get_logger("GoToArea4red"), "Failed to get ROS2 node instance");
        ...
        if (!target_pose) {
            RCLCPP_ERROR(node_->get_logger(), "Failed to get Area4red target position: target_pose");
            return BT::NodeStatus::FAILURE;
        }
        ...
        RCLCPP_INFO(node_->get_logger(), "Start navigating to area4red (x=%.2f, y=%.2f), tolerance=%.2fm",
    ...
    BT::NodeStatus onRunning() override
    {
        ...
        if (std::chrono::steady_clock::now() > timeout_point_) {
            RCLCPP_WARN(node_->get_logger(), "Navigation to Area4red timed out");
            return BT::NodeStatus::SUCCESS;
        }

        // 仅按当前点到目标点距离判断到达（不依赖 navigation/status）
        if (checkDistanceToTarget()) {
            RCLCPP_INFO(node_->get_logger(), "Reached Area4red");
            return BT::NodeStatus::SUCCESS;
        }
    ...
    {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Navigation to Area4red interrupted");
        } else {
            RCLCPP_INFO(rclcpp::get_logger("GoToArea4red"), "Navigation to Area4red interrupted (node not initialized)");
        }
        navigation_started_ = false;
    }
