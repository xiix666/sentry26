...
/**
 * @brief 前往区域4的异步行为节点
 * 导航到区域4并持续监控导航状态，距离目标点<=0.4m视为到达
 */
class GoToArea4 : public BT::StatefulActionNode
{
public:
    GoToArea4(const std::string& name, const BT::NodeConfig& config)
    ...
    {
        return {
            BT::InputPort<geometry_msgs::msg::PoseStamped>("target_pose", "Area4 position"),
    ...
    BT::NodeStatus onStart() override
    {
        ...
        if (!node_input) {
            RCLCPP_ERROR(rclcpp::get_logger("GoToArea4"), "Failed to get ROS2 node instance");
        ...
        if (!target_pose) {
            RCLCPP_ERROR(node_->get_logger(), "Failed to get Area4 target position: target_pose");
            return BT::NodeStatus::FAILURE;
        }
        ...
        RCLCPP_INFO(node_->get_logger(), "Start navigating to area4 (x=%.2f, y=%.2f), tolerance=%.2fm",
    ...
    BT::NodeStatus onRunning() override
    {
        ...
        if (checkDistanceToTarget()) {
            RCLCPP_INFO(node_->get_logger(), "Reached Area4");
            return BT::NodeStatus::SUCCESS;
        }
    ...
    {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Navigation to Area4 interrupted");
        } else {
            RCLCPP_INFO(rclcpp::get_logger("GoToArea4"), "Navigation to Area4 interrupted (node not initialized)");
        }
        navigation_started_ = false;
    }
...
private:
    bool checkDistanceToTarget()
    {
        ...
        RCLCPP_DEBUG(node_->get_logger(), "Current position(%.2f, %.2f), target position(%.2f, %.2f), distance=%.2fm, tolerance=%.2fm, result=%s",
